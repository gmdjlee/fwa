...
> **[정정 2026-08-01 · #32 검토]** 위 「런처 아이콘 1개」는 오기다. S1 이후에도 릴리스에는
> `PanelActivity`("FW Panel")와 `OnboardingActivity`("FoldWindow") **2개**가 남는다(릴리스 병합
> 매니페스트 실측). 1개로 줄이는 것은 별도 작업이며 설계 = `docs/DESIGN_32_LAUNCHER_ENTRY.md`.
> 또한 이 격리는 **매니페스트 선언만** 옮긴 것이라 `probe/` Kotlin 소스는 `main` 에 남아
> 릴리스 APK 에도 컴파일돼 들어간다(`isMinifyEnabled = false`). 사라진 것은 도달 가능성이지
> 바이트코드가 아니다 — `probe/` 완전 삭제는 아래 M5 2단계로 이월된 그대로다.
...
### W3 — 기하 정합성 · 도메인 불변식 · **[완료 2026-07-29 · `8edcce3`]**
