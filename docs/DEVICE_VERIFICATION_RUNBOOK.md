...
v1 에 **필수인 항목은 없다** — 아래는 전부 자연 발생 대기 중인 희귀 경로다(`PROGRESS.md` 는 v1 잔여
0 으로 종결됐고, 이 절이 목록의 유일한 소유자다). 자연 발생하거나 새 유도 후보가 생기면 그때 판정한다.
...
3. **`PROGRESS.md`** 를 Advisor 가 갱신한다 — 상태 헤더와 결정 로그. 새로 **수행이 필요한** 작업이
   생겼을 때만 항목을 만든다(선택 항목은 이 런북 §5 에 둔다).
