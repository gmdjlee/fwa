...
수정본(`awaitSettledSplitState` 포함) 설치·접근성 재활성화 완료 · 회전 `user-rotation free` 복원 완료(lock 잔재는 FoldingFeature 판정을 오염시킨다 — 개발 환경 함정) · `wm size`/`wm density`(override 360) 정상 · 애니메이터 배율 1.0 · TalkBack 해제 · 전체화면 자동 토글 켜짐 · 유튜브 분할 활성(패널 상단) · 버블 실행 중.
