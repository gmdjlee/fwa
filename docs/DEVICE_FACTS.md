...
## 분할 진입 전략 (#6 FAILS → Recents 폴백)

`GLOBAL_ACTION_TOGGLE_SPLIT_SCREEN` 이 막혔으므로 **P2-3은 Recents 폴백이 기본**.

### Recents 분할 진입 레시피 ([측정] — adb로 전 과정 검증 성공)

접근성 트리에서 각 단계가 안정적으로 조회됨. P2-3 구현의 직접 근거.

| 단계 | 동작 | 셀렉터 / 노드 |
|---|---|---|
| 1 | Recents 열기 | `GLOBAL_ACTION_RECENTS` 상당 = `KEYCODE_APP_SWITCH` |
| 2 | 대상 앱 카드의 아이콘 노드 탭 | `content-desc = "고급 옵션, <앱이름>, 버튼"`, 패키지 = launcher(`com.sec.android.app.launcher`) |
| 3 | 팝업 메뉴에서 "분할 화면으로 열기" 탭 | `text = "분할 화면으로 열기"` |
| 4 | 파트너 선택 UI에서 파트너 앱 탭 | launcher `FromRecentActivity` 내 앱 노드 |

> content-desc/text 문자열은 **언어 설정 의존**. 한국어 기준값이며 다국어는 [미검증]. 셀렉터는 문자열 하드코딩 대신 패키지+역할 기반 폴백을 두는 것을 권장.
