...
**최종 갱신:** 2026-07-28 16차 (**② 소규모 실기기 확인 3항목 완료 + 타이머 상향**: 알림 권한 플로우 회수→카드 감지→시스템 다이얼로그→허용→FGS 알림 실물(id=1001) 전 구간 · 중지 레이스 **물리 폴드 접기** 완주(§26⑤ 잔여 해소 — FGS 소멸·enabled=false 영속·에러 0) · 제스처 실사용감 무불만·오분류 0(시스템 표준 임계 확정) · `HIDE_SAFETY_TIMEOUT_MS` 30s→**90s**(이론 최악 MENU 경로 ≈70s > 30s — 조기 복원은 세션 중 오버레이 재출현 = 함정 #22 자충수. 실측 최장 12s). DEVICE_FACTS 16차 절. 직전 15차: **P3-5 실기기 검증 5항목 전부 통과 — 캠페인 중 결함 2건 발견·수정, qa PASS 230/230**. ① UiContext: 서비스 자신·2-인자 createWindowContext 전멸 (assertUiContext 거부/display 미연결) → **3-인자 `createWindowContext(display, TYPE_ACCESSIBILITY_OVERLAY, null)` 채택**, 방출 수신 확인. ② 노트북 자세 HALF_OPENED_HORIZONTAL + hingeBounds `Rect(0,984-2184,984)` 실측 — orientation 은 **창 상대** 의미론(회전 잠금이면 물리 자세 무관 VERTICAL). ③ 닫기 오발화: 반열림 체류 실측 ~2s(3표본)로 "800ms 미만 통과" 가정 반증, +803ms 오발화 물증 → **2층 방어 구현**: 힌지 각도 안정성 게이트(`TYPE_HINGE_ANGLE` 실노출 확인, 대역 45~135 ∧ 침묵 600ms ∨ 스프레드 ≤8°, 정상 닫기 2/2 차단) + FLEX 세션 자세-이탈 취소(`posture-exit` → CANCELLED 실증, Done 후엔 유지). ④ E2E 접기→완료 7.1s, `placementSource=FLEX` TOP residual=0 픽셀 물증, 2회 재현. ⑤ 기준선 OVERRIDE/BOTTOM → FLEX TOP 후 수동 = `LAST_SUCCESS/BOTTOM` — 비오염 실증. qa CONDITIONAL 1건은 Advisor 확정으로 해소: FLEX 라벨 = "자세가 위치를 자동 결정"(트리거 출처 무관), 근거 소멸 시 취소가 정합 — OVERRIDE 만 불가침. DEVICE_FACTS 15차 절 = SSOT)
**현재 Phase:** Phase 3 — P3-1~P3-5 **전부 완료** (P3-5 실기기 검증 포함). 잔여 = 미발동 경로 [미검증] 목록뿐
**다음 행동 (착수 목록):**
1. 16차 커밋 (사용자 승인 후): FloatingLauncherService 타이머 90s / DEVICE_FACTS 16차 / PROGRESS
...
3. #20 잔여 미발동 경로는 자연 발생 대기 (mech 로그 상시 계측) — 3시도 전멸 재발 시 FORENSIC viewId 로 특정 후 스텝 되감기 재검토
