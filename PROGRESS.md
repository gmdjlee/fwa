...
**최종 갱신:** 2026-07-28 **Phase 4 구현 세션** — P4-2(파트너 위젯: 시계/메모/검정 + DataStore 영속) · P4-3(커버 전환 자동 해제: `CoverDismissPolicy` 디바운스 600ms→재검증→게이트 4종→패널 직접 finish) · P4-4(앱 페어 pinned shortcut: 트램펄린 + 전면 대기 150ms/5s 사전 폴링) **구현 완료**. Worker 3기 병렬/순차 위임, 결함 0, qa **CONDITIONAL**(정적 전 항목 PASS — 257 테스트·빌드·규칙/계약 회귀 0 · 유일 차단 = 실기기 미검증). DEVICE_FACTS 「Phase 4 구현분」 절 = 17차 체크리스트. P4-1 freeform 은 `docs/DESIGN_P41_FREEFORM.md` 작성 — **프로브 F1~F6 통과 전 구현 금지** 확정(기기 미연결로 프로브 미실시). 커밋 `9c36905` 푸시 완료 — 재개 지점 = 아래 「다음 행동」 1·2번
**직전(16차):** 커밋 `e7af817` (**② 소규모 실기기 확인 3항목 완료 + 타이머 상향**: 알림 권한 플로우 회수→카드 감지→시스템 다이얼로그→허용→FGS 알림 실물(id=1001) 전 구간 · 중지 레이스 **물리 폴드 접기** 완주(§26⑤ 잔여 해소 — FGS 소멸·enabled=false 영속·에러 0) · 제스처 실사용감 무불만·오분류 0(시스템 표준 임계 확정) · `HIDE_SAFETY_TIMEOUT_MS` 30s→**90s**(이론 최악 MENU 경로 ≈70s > 30s — 조기 복원은 세션 중 오버레이 재출현 = 함정 #22 자충수. 실측 최장 12s). DEVICE_FACTS 16차 절. 직전 15차: **P3-5 실기기 검증 5항목 전부 통과 — 캠페인 중 결함 2건 발견·수정, qa PASS 230/230**. ① UiContext: 서비스 자신·2-인자 createWindowContext 전멸 (assertUiContext 거부/display 미연결) → **3-인자 `createWindowContext(display, TYPE_ACCESSIBILITY_OVERLAY, null)` 채택**, 방출 수신 확인. ② 노트북 자세 HALF_OPENED_HORIZONTAL + hingeBounds `Rect(0,984-2184,984)` 실측 — orientation 은 **창 상대** 의미론(회전 잠금이면 물리 자세 무관 VERTICAL). ③ 닫기 오발화: 반열림 체류 실측 ~2s(3표본)로 "800ms 미만 통과" 가정 반증, +803ms 오발화 물증 → **2층 방어 구현**: 힌지 각도 안정성 게이트(`TYPE_HINGE_ANGLE` 실노출 확인, 대역 45~135 ∧ 침묵 600ms ∨ 스프레드 ≤8°, 정상 닫기 2/2 차단) + FLEX 세션 자세-이탈 취소(`posture-exit` → CANCELLED 실증, Done 후엔 유지). ④ E2E 접기→완료 7.1s, `placementSource=FLEX` TOP residual=0 픽셀 물증, 2회 재현. ⑤ 기준선 OVERRIDE/BOTTOM → FLEX TOP 후 수동 = `LAST_SUCCESS/BOTTOM` — 비오염 실증. qa CONDITIONAL 1건은 Advisor 확정으로 해소: FLEX 라벨 = "자세가 위치를 자동 결정"(트리거 출처 무관), 근거 소멸 시 취소가 정합 — OVERRIDE 만 불가침. DEVICE_FACTS 15차 절 = SSOT)
**현재 Phase:** Phase 4 **진행 중** — P4-2·P4-3·P4-4 구현 완료(**실기기 미검증**), P4-1 은 프로브 선행 대기. Phase 3 는 완료 확정
**다음 행동 (착수 목록):**
1. **17차 실기기 캠페인** — Phase 4 구현분 검증: DEVICE_FACTS 「Phase 4 구현분」 절 체크리스트(P4-3 7항목 · P4-2 5항목 · P4-4 5항목). 최우선 미지수 = P4-3 항목 2(닫힘 상태에서 패널 finish 의 분할 해소 실효성)
2. P4-1 프로브 F1~F6 (`docs/DESIGN_P41_FREEFORM.md`) — adb 만으로 수행 가능. F1·F2 중 하나라도 ❌ 면 P4-1 기각. **프로브 통과 전 구현 착수 금지**
3. P3-5 발 v1.5 후보: 포그라운드 안정성 윈도(폴드 전환 중 월렛 quick 카드가 event-tracked 오염 → 게이트5 통과 실측) · 재열기 멈칫 Done-후 분할 잔존(수동 해제로 복구, 키가드 게이트는 정당 사용례 훼손으로 기각) · 각도 대역 경계값 실측
4. #12 v1.5 후보 축적분: BOTH_AXES_BARS 시 보정 생략(G1 드리프트 실측) · 적응형 residual(글로우 필러박스 블라인드 G5 실측) · 비-16:9 콘텐츠 합치·캐시 실측 · flex 게이트2↔startArrange TOCTOU(이론상, DEVICE_FACTS 기록)
5. P3-2 잔여 [미검증]: dismissSplit 인텐트 폴백(instance null 희귀 경로). 회전×2 폴백 검증 기회 확보
6. #20 잔여 미발동 경로는 자연 발생 대기 (mech 로그 상시 계측) — 3시도 전멸 재발 시 FORENSIC viewId 로 특정 후 스텝 되감기 재검토
