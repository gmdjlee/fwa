...
| 버블 링 55% 시인성 | ✅ (수정 후) 순검정 위 3.238:1 AA 통과 — 아래 E |
...
### E — 버블 유휴 알파 대비: ✅ 24차에서 해소 (스트로크 `#8C` → `#A0`)

발견 시 순검정 위 링 **2.96:1** 로 WCAG 1.4.11(비텍스트 3:1)에 0.04 미달이었다. **뷰 알파(0.65→0.66)
대신 링 스트로크 알파만 올렸다** — 뷰 알파 안은 여유가 0.02뿐이라 반올림 한 단위에 무너지고, 원까지
밝아져 "영상 옆 발광 억제"라는 유휴 페이드의 목적을 갉아먹는다. 스트로크만 올리면 **원 휘도 불변**이다.

실기기 픽셀 재계측(패널 BLACK 모드 = 순검정, 무접촉 8초 후): 링 **(81,98,88) → 3.238:1 AA 통과**,
원 (32,47,39) → 1.497:1 **불변**. 예측치와 채널당 1 이내 일치. 상세 = `docs/DEVICE_FACTS.md` 24차
「항목 #5 후속」 절.

- **잔여(v1.5)**: 중간 휘도대는 최선 1.48 로 여전히 낮다. 반투명 세이지 톤의 구조적 한계라 알파로는
  해소 불가 — 대비 윤곽(예: 어두운 아웃라인 1px 동반)이 필요하다.
...
| 2026-08-01 24차 | 버블 링 AA 미달은 **뷰 알파가 아니라 스트로크 알파**로 해소(`#8C`→`#A0`) | 뷰 알파 0.66 은 여유 0.02 로 반올림 한 단위에 무너지고 **원까지 밝아져** 유휴 페이드의 목적(영상 옆 발광 억제)을 훼손한다. 스트로크만 올리면 원 휘도 불변이고, 밝은 배경에서는 원이 분리를 담당하므로 실질 분리도가 나빠지는 구간이 없다 |
