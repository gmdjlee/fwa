...
**최종 갱신:** 2026-07-28 **P4-1 프로브 세션** — 프로브 F1~F6 adb 단독 수행, **게이트 통과** (F1·F2·F3·F4·F6 ✅ / F5 DRM 재생만 수동 잔류 → 17차 편입). One UI 팝업 = freeform(5) 확정(팝업 크롬 + `isAlwaysOnTopFreeform=true` 실측), `am start --windowingMode 5` 실행·기존 태스크 전환 성공, `am task resize` 픽셀 정확(오차 0), UNRESIZEABLE 넷플릭스 **`enable_non_resizable_multi_window=0` 에서도 진입** — 셸 경로는 설정 비의존, a11y 노출 = `APPLICATION` 타입 bounds 1:1(검증 폴링 가능). **후보 A(Shizuku 셸) 채택, 후보 B 기각 — P4-1 구현 착수**. Phase 4 빌드(`9c36905`) 기기 설치 완료 — 17차 캠페인 즉시 가능 상태. DEVICE_FACTS 「P4-1 프로브 F1~F6」 절 = SSOT
...
**현재 Phase:** Phase 4 **진행 중** — P4-2·P4-3·P4-4 구현 완료(**실기기 미검증**), P4-1 프로브 통과 → 구현 착수. Phase 3 는 완료 확정
**다음 행동 (착수 목록):**
1. **17차 실기기 캠페인** — Phase 4 구현분 검증: DEVICE_FACTS 「Phase 4 구현분」 절 체크리스트(P4-3 7항목 · P4-2 5항목 · P4-4 5항목) + **P4-1 F5(DRM 팝업 재생) 1항목**. 최우선 미지수 = P4-3 항목 2(닫힘 상태에서 패널 finish 의 분할 해소 실효성). 빌드는 이미 기기 설치됨(프로브 세션에서 `installDebug` 완료, a11y = Arranger 단독 활성 확인)
2. **P4-1 구현** (프로브 통과, 후보 A = Shizuku 셸 채택) — Shizuku 의존성 + `PopupPlanner`(domain) + 서비스 팝업 경로 + 버블 메뉴 항목 + 온보딩 카드. `docs/DESIGN_P41_FREEFORM.md` §4 스케치 기준
