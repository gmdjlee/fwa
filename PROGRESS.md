...
**E**(버블 링 AA)도 해소. **F**(TalkBack) 는 전제 반증 + 플랫폼 제약 확인 후 **v1 범위 밖으로 종결**.
잔여는 선택 항목(B/C) 과 원인 미상 재조사 대기(D) 뿐이다.
...
| **TalkBack 롱클릭 메뉴 진입**(F1) + 토글 낭독 | 🚫 **범위 밖 종결** — 아래 F |
| 패널 메모 좌상단 무음 정지 소멸(F6c) | 🚫 **범위 밖 종결** — 아래 F |
...
### F — TalkBack 항목: 🚫 v1 범위 밖으로 종결 (24차, 사용자 승인)

`TASK.md` 「범위 밖」에 명시했다. 근거·측정은 `docs/DEVICE_FACTS.md` 24차 「F 항목(TalkBack) 종결」 절.

**① F1 의 설계 전제가 반증됐다.** 사용자 손가락 실측: TalkBack 활성 시 버블 **단일 탭**은 TalkBack 이
정상 낭독하고 **앱에는 터치가 도달하지 않는다**(앱 로그 전무 = 이 오버레이는 터치 탐색 **대상 안**).
그런데도 같은 세션에서 메뉴는 열렸고 로그는 `bubble long-press`(**터치 경로**)였다 —
`bubble long-click(접근성 경로)` 은 **세 세션 통틀어 0회**. 평범한 터치가 앱에 안 닿는데 터치 롱프레스가
발화했다는 건 TalkBack 이 어떤 제스처에서 터치를 **통과시킨다**는 뜻이다(두 번 탭 후 유지가 유력, 제스처
종류는 미확정). 즉 **「메뉴 도달 경로가 아예 없었다」는 성립하지 않는다** — F1 이전에도 도달 가능했다.
KDoc 을 「유일한 경로」 → 「명시적 경로」로 정정했다(**코드는 무변경**).

**② 그보다 무거운 플랫폼 제약.** **TalkBack 이 켜져 있으면 앱이 분할을 생성하지 못한다** —
`SplitEntry` step2 의 Recents 카드 드래그가 **3/3 실패**(카드 아이콘 탐색은 성공, 드래그만 거부됨.
TalkBack UI 를 홈으로 정착시킨 교란 통제 시행 포함). TalkBack OFF 대조군은 성공. 분할을 먼저 만들고
TalkBack 을 켜는 우회도 안 된다 — 켜는 순간 포그라운드가 바뀌며 분할이 풀린다(2/2).
**메뉴 접근성을 완성해도 그 메뉴가 하는 일이 완주하지 못한다.**

**결정**: F-1·F-2·F-3 을 [미검증]이 아니라 **범위 밖으로 종결**. 접근성 코드(라벨·`isCheckable`/
`isChecked`·`clearAndSetSemantics`·`setOnLongClickListener`)는 **전부 유지** — 저시력/확대 사용자에게
이득이고 제거가 오히려 회귀 위험이다.

**v1.5 재검토 시 선행 질문 하나**: 터치 탐색 활성 시 Recents 카드 드래그가 왜 거부되는가, 우회가 있는가.
그게 풀리지 않으면 나머지 접근성 작업은 의미가 없다.
...
| 2026-08-01 24차 | **스크린리더(TalkBack) 지원을 v1 범위 밖으로 명시.** F-1·F-2·F-3 을 [미검증]이 아니라 「범위 밖」으로 종결하고, 접근성 코드는 전부 유지 | ① 앱의 가치가 시각적이라 대상 사용자가 사실상 부재(저시력용 대비·확대·터치 타깃은 이미 확보) ② ADR-6 으로 배포 강제 없음 ③ **TalkBack 활성 시 분할 생성 자체가 불가**(Recents 카드 드래그 3/3 실패) — 메뉴 접근성을 완성해도 그 메뉴가 하는 일이 완주하지 못한다. 코드를 지우지 않는 이유는 저시력/확대 이득 + 제거가 회귀 위험이기 때문 |
| 2026-08-01 24차 | F1 KDoc 의 「유일한 경로」 → 「명시적 경로」 **정정**(코드 무변경) | 실측이 전제를 반증했다 — 오버레이는 터치 탐색 대상 안인데도 메뉴가 터치 경로로 열렸다(= 통과 동작 존재). 「도달 방법이 아예 없었다」가 거짓이었다. 직전 캠페인의 지배적 결함이 「주석이 사실과 다름」이었으므로 범위 결정과 무관하게 고친다 |
