...
**②** 가로 최소 페인이 563px 인데 상수가 181 → 실측 교체 + 클램프 고지(「4:3 구형」 무고지 해소).
**E**(버블 링 AA)도 해소. **F**(TalkBack) 는 전제 반증 + 플랫폼 제약 확인 후 **v1 범위 밖으로 종결**.

### 🏁 v1 잔여 작업 0

수행이 **필요한** 항목은 남아 있지 않다. 종전 「남은 작업」 절에 있던 비필수 항목 — 희귀 경로
[미검증] 자연 발생 대기 · v1.5 후보 · 원인 미상 관측 — 은 이 문서에서 제거했다. 근거와 재현 로그는
전부 다른 SSOT 에 이미 있고(`docs/DEVICE_FACTS.md`, 희귀 경로 유도 절차는
`docs/DEVICE_VERIFICATION_RUNBOOK.md`, 결정 요약은 아래 결정 로그), 목록 자체가 다시 필요해지면
git 이력에서 복원한다 — `git show 481c066:PROGRESS.md`.
...
**최종 트리 상태: 단위 테스트 373/373 ✅ · assembleDebug ✅ · lintDebug 신규 이슈 0 ✅**
...
| 버블 링 55% 시인성 | ✅ (수정 후) 순검정 위 3.238:1 AA 통과 — 스트로크 알파 `#8C`→`#A0` |
| **TalkBack 롱클릭 메뉴 진입**(F1) + 토글 낭독 | 🚫 **범위 밖 종결** — `TASK.md` 「범위 밖」 |
| 패널 메모 좌상단 무음 정지 소멸(F6c) | 🚫 **범위 밖 종결** — `TASK.md` 「범위 밖」 |
...
| #32 런처 진입점 통합 검토 | ✅ 검토 완료(코드 변경 0) | 결론은 아래 결정 로그(2026-08-01) 참조 |
...
4. `minPaneHeight` 실측 → **24차에 해소·정정.** 가로 상하분할 최소 페인 = **563px**(= 250dp), 양쪽 페인 대칭(상한 1391 = 1954−563). **종전의 181 은 「최소 페인」이 아니라 접힘 슬라이버의 가시 폭**이었고 181~563 은 존재하지 않는 대역이다 — 그 대역을 요청하면 플랫폼이 조용히 563 으로 되돌린다. `SplitPlanner` 상수는 563 으로 교체 완료(24차, `docs/DEVICE_FACTS.md` 「결함 ② 수정」 절). 세로 좌우분할 값은 이 앱이 쓰지 않으므로(기하는 `foldSevenLandscape()` 하나뿐) 재측정하지 않았다
...
13. 필러박스 맹점(과소 이동 시 `residualBars=0` 오판) → 열축 로직은 confirm 측정과 동일 기반, v1 은 `residualColumns` 로그 보고까지만(`verified` 의미론 반영은 v1.5). 23차: 오탐 원인은 "미구현"이 아니라 "미연결"로 판명 — `residualBars()` 에 `detectHybrid()` 의 적응형 폴백만 연결하면 된다(v1.5, `docs/DEVICE_FACTS.md` 23차 절)
...
19. 회전 결과 페인 위치 비결정 — 원인 미상. 누적 TOP 10 / BOTTOM 2(상단 편향). 교정 체인 = `PaneSwapper`(수렴) → 회전×2 폴백([미발동], 누적 12+ 세션) → 실패 시 하단 유지+토스트
20. 클릭-사이클 에스컬레이션 → 10차 Gate 1~3 통과로 주 경로 완결(15/15 done, `ENTRY_STEP_FAILED` 0). 잔여 = cycle-2(a11y)·스왑 제스처 사이클 [미발동] — `mech` 로그로 상시 계측, 유도 절차는 `docs/DEVICE_VERIFICATION_RUNBOOK.md`
...
| 2026-08-01 | `#32` 런처 진입점 통합 = 「합치기」 불가, 「소유권 이전」만 가능. L2(A/B 인과 확정) 선행 후 착수 | `PanelActivity` 가 finish 경로 5개·sticky 래치·prune 판정 전부의 유일 전제. 성립 가능한 유일한 방향 = `OnboardingActivity` 의 LAUNCHER 제거(선행 = 「피커 노출이 LAUNCHER 때문인가」 실기기 A/B). v1.5 |
