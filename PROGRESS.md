...
**최종 갱신:** 2026-07-26 12차 (**#12 §6 측정 캐싱 v1 구현 완료** — admission = 합치∧verified. 티어 PROFILE>MEASURED>**CACHED**>PRESET (`AspectSource.CACHED` 신설, JSON 금지), 적용 2지점 = beginSession resolve + finishConfirm 불합치 폴백(cached 우선). confirm 발동 조건·머신·시드 JSON·검증 상수 전부 무변경. requireAgreement=false 롤백 세션은 confirm 미실행이라 캐시 저장도 자동 차단 — 단일 프레임 값 유입 경로 부재(qa 전수 확인). 무 TTL·범위 검증 양측·롤백 레버 `defaults.cacheMeasuredAspect`(부재=true). 테스트 186→204, qa-verifier PASS 결함 0. 실기기 [미검증] — DEVICE_FACTS 미검증 표 4항목. 미커밋)
**현재 Phase:** Phase 3 진행 중 — P3-1·P3-2·P3-3·P3-4 완료, 잔여 P3-5
**다음 행동 (착수 목록):**
1. #12 §6 측정 캐싱 실기기 검증 (DEVICE_FACTS 미검증 표: 합치 세션 후 저장 로그·키 확인 → CACHED 낙착 → 자기 갱신 부재 → 레버 false 회귀). P3-5 FoldingFeature
2. #12 v1.5 후보 축적분: BOTH_AXES_BARS 시 보정 생략(G1 드리프트 실측) · 적응형 residual(글로우 필러박스 블라인드 G5 실측) · 비-16:9 콘텐츠 합치 실측
