...
**다음 행동 (착수 목록):**
1. **step3 패널 소환 결함 수정** (최우선 — P4-3 활성 상태의 일상 재생산 결함). **설계 확정 + 프로브 선통과: `docs/DESIGN_27_PANEL_CARD.md`** — 18차 adb 프로브(코드 무변경)로 **G1·G3 통과 + purge 자충 실측 재현** 완료. 주 수정은 **축 A(파괴 제거)**: A1 `finishAndRemoveTask`→`finish` 4곳 격하, A2 `purgeStalePanelTasks`→`pruneExtraPanelTasks`(MRU 1개 보존). 축 B(소환)는 카드 0 상태 안전망으로 **`ActivityManager.addAppTask()`(B0)** 1순위. 인접 결함 #28 은 AOSP 소스 확인으로 [확정] — 동반 수정. 잔여 프로브 = G2·G4·G5·G6·G7
...
| Phase 4 확장 | 🔄 검증 완료·결함 수정 대기 (#27 원인 확정·설계 확정, 구현 착수 가능) | P4-1·2·3·4 구현 + **17차 실기기 검증 완료** (P4-2 5/5 · P4-3 6/7 에뮬 · P4-4 4.5/5 · P4-1 7/7, DEVICE_FACTS 17차 절). 잔여 = **step3 패널 소환 결함 수정(필수)** + Phase B 물리 3건(물리 접기·DRM 육안) |
