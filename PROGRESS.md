...
**최종 갱신:** 2026-07-25 저녁 11차 (**#12 신뢰도 필터 검토 완결 + 구현 완료**: 설계 = 단일 프레임 conf 폐기(오염 conf 0.60~0.97 실측) → **2-샷 합치 게이트** (pre 행축 × 진입 후 confirm 열축, 3% 합치 시만 MEASURED, 불합치 PRESET 폴백). 구현 = Worker 2명 순차(도메인+파서 → 샘플러+서비스) + qa-verifier **PASS**: 182/182 테스트(+25)·빌드 성공·머신 diff 0·domain 순수성 0건 위반·§7 IN 전항목 실재 확인. aspectOverride tier 0 승격(보정도 생략) 포함. 상세 `docs/DESIGN_12_MEASUREMENT_CONSENSUS.md`. **실기기 G1~G5 [미검증]**, 미커밋)
**현재 Phase:** Phase 3 진행 중 — P3-1·P3-2·P3-3·P3-4 완료, 잔여 P3-5
**다음 행동 (착수 목록):**
1. #12 실기기 검증 **G1~G5** (DESIGN_12 §9: G1 영상 시작 직후 탭=1.6 사고 재현 → 불합치·PRESET 기대 / G2 컨트롤 노출 중 탭 / G3 클린 3회 합치·지연 ≤+0.5s / G4 앰비언트 ADAPTIVE 합치 유지 / G5 메뉴 프리셋 tier 0 로그) → 통과 시 측정 캐싱(§6, admission=합치∧verified) 재검토. P3-5 FoldingFeature
...
| `domain/MeasurementConsensus.kt` | ✅ 11차 #12 신규. 순수 Kotlin — 2-샷 합치 판정 (`classifyAxis`/`classifyConfirm`/`agree`, verdict 9종, DESIGN_12 §3.3 표의 코드화). 테스트 21개. 실기기 [미검증] |
...
| `data/WindowProfilesParser.kt` | ✅ P1-2 신규. kotlinx-serialization DTO→domain 매핑, 예외 누출 없이 `ProfilesParseResult`. 11차 #12: `requireMeasurementAgreement` 토글 (키 부재=true, 1줄 롤백 레버 — 시드 JSON 무수정). 테스트 17개(실제 SSOT 파일 파싱 포함) |
...
| `platform/ScreenshotSampler.kt` | ✅ v2 확장. 행별 luma 평균/분산 산출 추가. 11차 #12: `toPillarboxScan` 열축 전치판 (entries=열, width 자리=height/colStride — 역산 좌표계 계약) 추가, 기존 함수 무변경. Bitmap 의존이라 JVM 테스트 불가 — 실기기 미검증 |
...
12. 사전 실측 오염: 플레이어 컨트롤 오버레이/앰비언트 글로우가 떠 있으면 종횡비 오측 (1.333/1.12 관측. **추가 실측 2026-07-25 오후 2차: 영상 시작 직후 탭 → 추천화면/인트로 오염 1.6 오측(conf 0.60), 어두운 장면이라 verify residual=0 오판**). → **11차 검토 완결 + 구현 완료** (qa PASS 182/182): 2-샷 합치 게이트 (`docs/DESIGN_12_MEASUREMENT_CONSENSUS.md`). 오염원 전부 일시적 ↔ 진짜 띠는 지속 — 시간·축·컨텍스트 분리된 두 측정의 합치만 신뢰. **실기기 G1~G5 [미검증]**. 알려진 좁은 갭(코드 무변경, 관찰만): 재트리거 시 디바이더가 이미 목표 4px 이내면 머신이 Dragging 스킵 → confirm 미발동 (pre 단독 = 종전 동작과 동일, 회귀 아님 — pre 오측이면 목표가 현 위치와 어긋나 Dragging 을 타므로 자연 커버)
13. 필러박스 맹점: 과소 이동 시 `residualBars=0` 으로 verified 오판 가능. → 열축 도메인 로직이 #12 confirm 측정과 동일 기반 — v1 에서 `residualColumns` **로그 보고까지** 동봉, `verified` 의미론 반영은 v1.5 (DESIGN_12 §3.4·§7)
