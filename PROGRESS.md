...
**최종 갱신:** 2026-07-25 저녁 9차 (**#20 클릭-사이클 에스컬레이션 구현 완료 — 실기기 [미검증]**. 파일 7종: ClickCyclePlan 신규(순수 도메인+테스트 12) · step3/menuStep4 사이클 재작성 + LAUNCH_ADJACENT 삭제 · PaneSwapper 정착 게이트+스위치 클릭 사이클 · 서비스 배선(awaitDividerSettled·오버레이 람다·TYPE_VIEW_CLICKED 포렌식) · 이벤트 마스크. 153 테스트 green + assembleDebug green, qa-verifier 독립 검증 **PASS** (A~H 전항목, 검증 상수 동결·로그 계약·ADR-2 확인). 미커밋)
**현재 Phase:** Phase 3 진행 중 — P3-1·P3-2·P3-3·P3-4 완료, 잔여 P3-5
**다음 행동 (착수 목록):**
1. **#20 실기기 검증 Gate 1~3** (`docs/DESIGN_20_CLICK_CYCLE.md` §5): Gate1 회귀(유튜브 DRAG ×3 + 넷플릭스 MENU ×3) → Gate2 독 컨텍스트(무override 연속 ≥5회 ENTRY_STEP_FAILED 0건 기준 + 회전 여파 스왑 ≥3회) → Gate3 logcat mech 집계 → DEVICE_FACTS 신규 절
2. #12 신뢰도 필터 검토 → 이후 측정 캐싱 재검토. P3-5 FoldingFeature
...
| `domain/ClickCyclePlan.kt` | ✅ #20 신규 (9차). 순수 Kotlin — 클릭 메커니즘 사이클 계획. `PICKER`(gesture-first)/`POPUP_SWITCH`(a11y-first) 프로파일 = 데이터(1줄 롤백 레버), `mechanismFor` 클램프, verifySlice≥400ms 불변식. 테스트 12개 (메커니즘 순서 동결 회귀 포함). 실기기 미검증 |
...
| `platform/PaneSwapper.kt` | ⚠️ 유튜브 세션 실기기 성공 / 넷플릭스 회전 직후 "창 전환" ACTION_CLICK 무효 2회 실측 → **9차 #20 재구성 [실기기 미검증]**: 정착 게이트(`dividerSettled` 주입, 핸들 bounds 2연속 동일) + 스위치 클릭 = POPUP_SWITCH 사이클(a11y→gesture→gesture, involution 가드·팝업 소멸 시 재탭 전 isSwapped 재확인·검증 슬라이스 800ms). 핸들탭/팝업탐색 루프·더블탭 폴백 무변경. 실패 시 서비스 회전×2 폴백 유지 |
| `platform/SplitEntry.kt` | ✅ P2-3 **실기기 검증** (DRAG·MENU 양 경로). `EntryRecipe` 분기: DRAG 3단계(유튜브 회귀 E2E 통과) / MENU 5단계(UNRESIZEABLE 전용, E2E 6회 성공). 2026-07-25 오후 2차: step2 유령 매치 재폴링 + 목표 상태 선체크 수정. **9차 #20 [실기기 미검증]**: step3/menuStep4 = `clickUntilCondition` PICKER 사이클(gesture→gesture→a11y — 매 디스패치 직전 선체크·refresh 스테일 가드·오버레이 가드는 함수 전체 즉시 실패·dispatched=false 시 슬라이스 생략) 재작성, **LAUNCH_ADJACENT 전략2 삭제**. 생성자에 `ownOverlayVisible` 람다 주입. menuStep2/3/5·step1/2·셀렉터 무변경 |
